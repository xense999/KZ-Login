...
- **`accounts.gamania.com` 的 cookie 永遠不清。** 那是對方記住這台裝置的依據；清了，每次登入都得從頭驗一遍。要清的只有 beanfun 的（上次留下的 token 會讓 portal 短路掉這一次），而且要**確認刪掉了才注入、注入完才導向**，注入要用的位置不刪——刪除沒有完成通知、會晚到，細節歸 `browser`（見總表 `browser` 條的「注入之前先清」）。
