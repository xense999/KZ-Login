...
- `gamapass_login(account, password, region) -> { status: "approved", token, games } | { status: "cancelled" }`、`gamapass_code(code)`、`gamapass_cancel()`：GamaPass 帳號走遊戲橘子自己的登入頁，在一顆看不見的視窗裡進行（見總表 `gamapass` 條）。指令會一直 await 到登入完成或取消；期間的進度以 **`gamapass-stage` 事件**送給主視窗（`{ stage: "working" } | { stage: "code", sentTo, error, attempt } | { stage: "user" }`），事件名只寫在 `GAMAPASS_STAGE_EVENT`。`region` 是那顆視窗需要現身時要貼的位置，由登入頁量測。
