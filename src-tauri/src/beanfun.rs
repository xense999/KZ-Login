...
// ─── Debug：活動頁登入握手診斷（用完即刪） ────────────────────────────────────

/// Debug-only：用登入 session 的 cookie jar 去打活動頁，手動逐跳追 redirect，印出
/// 每一跳的網址／狀態碼／送出的 cookie／伺服器 Set-Cookie／Location，以及終點頁面
/// 命中的登入字樣。目的＝定位活動頁 SSO 握手在哪一步撲空。
///
/// 追蹤跑在 jar 的**深拷貝**上（伺服器沿途發的 Set-Cookie 只落在副本），所以完全
/// 不污染真正的登入 session。
#[cfg(debug_assertions)]
pub async fn diagnose_event_login(
    cookie_store: &Arc<CookieStoreMutex>,
    start_url: &str,
) -> Result<String, BeanfunError> {
    // 深拷貝 jar，追蹤過程不動到真 session。
    let work_store = {
        let src = cookie_store.lock()
            .map_err(|_| BeanfunError::Parse("jar poisoned".into()))?;
        Arc::new(CookieStoreMutex::new(src.clone()))
    };

    let client = Client::builder()
        .cookie_provider(work_store.clone())
        .user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36")
    ...
    let mut url = reqwest::Url::parse(start_url)
        .map_err(|e| BeanfunError::Parse(format!("URL 解析失敗：{e}")))?;

    for hop in 0..15 {
        let sent: Vec<String> = {
            let store = work_store.lock()
                .map_err(|_| BeanfunError::Parse("jar poisoned".into()))?;
            store.get_request_values(&url).map(|(n, _)| n.to_string()).collect()
        };
        ...
        let set: Vec<String> = resp.headers()
            .get_all(header::SET_COOKIE).iter()
        ...
        out.push_str(&format!("\n[hop {hop}] {status} {url}\n"));
        out.push_str(&format!("    送出 cookie：{}\n",
            if sent.is_empty() { "(無)".to_string() } else { sent.join(", ") }));
        if !set.is_empty() {
        ...
        if status.is_redirection() {
            if let Some(l) = loc {
                url = url.join(&l)
                    .map_err(|e| BeanfunError::Parse(format!("Location join 失敗：{e}")))?;
                continue;
            }
        }

        let body = resp.text().await.unwrap_or_default();
        let markers = ["尚未登入", "會員登入", "請登入", "身分驗證", "選擇帳號",
                       "QRCode", "qrcode", "掃描", "pSKey", "skey", "login"];
        let hits: Vec<&str> = markers.iter().filter(|m| body.contains(**m)).copied().collect();
        out.push_str(&format!("    終點：body {} bytes，命中登入字樣：{}\n",
            body.len(),
            if hits.is_empty() { "(無)".to_string() } else { hits.join(" / ") }));
        break;
    }

    // 追完 Login 握手後，看活動頁 Index 有沒有拿到登入 token（server 塞的 #sdEventToken）。
    {
        let idx = client
            .get("https://maplestory-event.beanfun.com/Event/E20260606/Index")
            .header(header::ACCEPT, "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
            .send().await?;
        let idx_body = idx.text().await.unwrap_or_default();
        let re = Regex::new(r#"id="sdEventToken"[^>]*value="([^"]*)""#).unwrap();
        let sd = re.captures(&idx_body).and_then(|c| c.get(1)).map(|m| m.as_str());
        out.push_str("\n=== Index 的 sdEventToken（活動頁登入 token）===\n");
        match sd {
            Some(v) if !v.is_empty() =>
                out.push_str(&format!("值：{}…（長度 {}）→ 活動頁已登入！\n", &v[..v.len().min(16)], v.len())),
            Some(_) => out.push_str("值：(空) → 活動頁未登入\n"),
            None => out.push_str("(頁面找不到 sdEventToken 欄位)\n"),
        }
    }

    out.push_str("\n=== 追蹤後（副本）jar 各域 cookie ===\n");
    {
        let store = work_store.lock()
            .map_err(|_| BeanfunError::Parse("jar poisoned".into()))?;
        for c in store.iter_unexpired() {
            let dom = match &c.domain {
                ...
                cookie_store::CookieDomain::Suffix(s) => format!(".{s}"),
                _ => "?".to_string(),
            };
            out.push_str(&format!("  {} {}\n", dom, c.name()));
        }
